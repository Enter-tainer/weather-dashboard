 ...
 */
import { SOUNDING_PRESSURE_LEVELS, APPROX_PRESSURE_HEIGHTS, dewPointFromRh } from './sounding';
import type {
  CloudLevel,
  MoonEvent,
  MoonEventList,
...
const PRESSURE_LEVELS = [1000, 925, 850, 700, 600, 500, 400, 300] as const;
const FALLBACK_ALT: Readonly<Record<(typeof PRESSURE_LEVELS)[number], number>> = {
  1000: 100,
  925: 750,
  850: 1500,
  700: 3000,
...
function sunAlt(hour: number): number {
...
function generateMembers(base: number, count: number, spread: number): number[] {
...
function generateWeatherCodeMembers(primaryCode: number, count = 10, agreement = 0.6): number[] {
  // Map of plausible alternative codes grouped by weather category
  const alternatives: Readonly<Record<number, readonly number[]>> = {
  ...
  for (let i = 0; i < count; i++) {
    if (Math.random() < agreement) {
      ...
      members.push(alts[Math.floor(Math.random() * alts.length)] ?? primaryCode);
    }
  }
  return members;
}

function makeCloudByLevel(lowCover: number, midCover: number, highCover: number): CloudLevel[] {
...
function makeBaseDate(dayOffset: number): Date {
  const d = new Date('2026-03-27T00:00:00');
  d.setDate(d.getDate() + dayOffset);
  return d;
}

function formatTime(baseDate: Date, hour: number): string {
...
function clearDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
    const isDay = alt > 0;
...
function cloudyDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
...
function rainyDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
    ...
    if (h < 6) {
      ...
      code = [51, 53, 55][Math.min(2, h - 6)] ?? 51; // drizzle: light→moderate→heavy
      precip = 0.2 + (h - 6) * 0.3;
      precipProb = 40 + (h - 6) * 15;
    } else if (h < 16) {
      code = [61, 63, 63, 65, 65, 65][h - 10] ?? 61; // rain: light→moderate→heavy
      precip = 1.5 + (h - 10) * 1.2;
      precipProb = 70 + (h - 10) * 5;
    } else if (h < 20) {
      code = [80, 81, 82, 82][h - 16] ?? 80; // rain showers: light→heavy
      precip = 3.0 + (h - 16) * 2;
      precipProb = 85 + (h - 16) * 3;
    } else {
      code = [63, 61, 3, 2][h - 20] ?? 2; // clearing
...
function snowDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
    ...
    if (h < 6) {
      ...
      code = 73;
      precip = 0.8 + rand(0, 0.5);
      precipProb = 75;
    } else if (h < 18) {
      code = 75;
      precip = 1.5 + rand(0, 1.0);
      precipProb = 90;
    } else if (h < 21) {
      code = 77;
      precip = 0.5 + rand(0, 0.5);
      precipProb = 65;
    } // snow grains
    else {
      code = 85;
      precip = 0.2 + rand(0, 0.3);
      precipProb = 40;
    } // snow showers
...
function fogDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
    ...
    items.push({
      ...
      visibility: vis,
      uvIndex:
        alt > 0 && code !== 45 && code !== 48
          ? Math.max(0, 3 * Math.sin((Math.PI * (h - 6)) / 12))
          : 0,
...
function thunderstormDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
...
function mixedDay(dayOffset: number): WeatherPoint[] {
  const base = makeBaseDate(dayOffset);
  const items: WeatherPoint[] = [];
  // Cycle through: clear → cloudy → drizzle → rain → thunderstorm → snow → fog → clear
  const hourCodes = [
    0, 0, 1, 2, 3, 3, 51, 53, 55, 61, 63, 65, 80, 81, 82, 95, 96, 75, 73, 71, 45, 48, 1, 0,
  ];
  const hourPrecip = [
    0, 0, 0, 0, 0, 0, 0.2, 0.5, 0.8, 1.0, 2.5, 5.0, 3.0, 5.0, 8.0, 10.0, 12.0, 1.5, 0.8, 0.3, 0, 0,
    0, 0,
  ];

  for (let h = 0; h < 24; h++) {
    const alt = sunAlt(h);
    const code = hourCodes[h] ?? 0;
    const precip = hourPrecip[h] ?? 0;
    ...
    items.push({
      ...
      cloudCover:
        precip > 0 ? Math.min(100, 60 + precip * 5) : code >= 2 ? 50 + rand(0, 30) : rand(0, 20),
...
export function generateMockTimeline(): WeatherTimeline {
  const generators: Array<(dayOffset: number) => WeatherPoint[]> = [
    clearDay,
    cloudyDay,
    rainyDay,
    snowDay,
  ...
  const allItems: WeatherTimeline = [];
  ...
  generators.forEach((gen, dayIdx) => {
    const items: WeatherPoint[] = gen(dayIdx).map((item) => ({
      ...item,
    ...
    if (moonriseHour < 24) {
      ...
      const moonriseEvent: MoonEvent = {
        type: 'moonrise',
        time: moonriseTime,
        localHour: moonriseTime.getHours(),
        localMinute: moonriseTime.getMinutes(),
        absoluteIndex: offset + moonriseHour,
      };
      if (firstItem?.moonPhase != null) moonriseEvent.phase = firstItem.moonPhase;
      if (firstItem?.moonFraction != null) moonriseEvent.fraction = firstItem.moonFraction;
      moonEvents.push(moonriseEvent);
    }
    if (moonsetHour < 24) {
      const moonsetTime = new Date(baseDate);
      moonsetTime.setHours(Math.floor(moonsetHour), Math.round((moonsetHour % 1) * 60), 0, 0);
      const moonsetEvent: MoonEvent = {
        type: 'moonset',
        time: moonsetTime,
        localHour: moonsetTime.getHours(),
        localMinute: moonsetTime.getMinutes(),
        absoluteIndex: offset + moonsetHour,
      };
      if (firstItem?.moonPhase != null) moonsetEvent.phase = firstItem.moonPhase;
      if (firstItem?.moonFraction != null) moonsetEvent.fraction = firstItem.moonFraction;
      moonEvents.push(moonsetEvent);
    }

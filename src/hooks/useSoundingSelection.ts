...
export function useSoundingSelection(data: WeatherPoint[] | null | undefined): SoundingSelection {
  ...
  const activeSoundingItem =
    Array.isArray(data) && soundingIndex >= 0 ? (data[soundingIndex] ?? null) : null;

  const setSoundingTime = useCallback((time: string | null) => {
    setSearchParam('sounding', time);
  }, []);

  const selectSoundingItem = useCallback(
    (item: WeatherPoint) => {
  ...
  const stepSounding = useCallback(
    (direction: number) => {
